-- =====================================================
-- OASIS BI PRO: FINAL DATABASE FIX
-- Created: 21 Desember 2025
-- Purpose: Fix foreign key constraint error dan Google OAuth
-- =====================================================

-- ===========================
-- PART 0: DROP PROBLEMATIC FOREIGN KEY CONSTRAINT
-- ===========================

-- Drop the foreign key constraint that causes the issue
-- user_profiles.customer_phone should NOT reference barbershop_customers
-- because user_profiles is created FIRST during registration
...
CREATE TABLE IF NOT EXISTS user_profiles (
  ...
  customer_phone TEXT, -- NO FOREIGN KEY - will be synced to barbershop_customers later
  customer_name TEXT,
  capster_id UUID, -- Will reference capsters table but added later
  full_name TEXT,
  user_role TEXT, -- Deprecated, use 'role' instead
...
ON capsters FOR ALL TO authenticated
USING (EXISTS (
  SELECT 1 FROM user_profiles 
  WHERE id = auth.uid() AND role = 'admin'
))
...
-- Now add the foreign key from user_profiles to capsters (safe order)
DO $$
BEGIN
  -- Check if constraint doesn't exist before adding
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'user_profiles_capster_id_fkey'
  ) THEN
    ALTER TABLE user_profiles 
    ADD CONSTRAINT user_profiles_capster_id_fkey 
...
ON service_catalog FOR ALL TO service_role
USING (true) WITH CHECK (true);
...
ON barbershop_transactions FOR ALL TO service_role
USING (true) WITH CHECK (true);

CREATE POLICY "customers_read_own_transactions"
ON barbershop_transactions FOR SELECT TO authenticated
USING (customer_phone IN (
  SELECT customer_phone FROM user_profiles 
  WHERE id = auth.uid() AND role = 'customer'
));
...
ON barbershop_transactions FOR ALL TO authenticated
USING (EXISTS (
  SELECT 1 FROM user_profiles 
  WHERE id = auth.uid() AND role IN ('capster', 'admin', 'barbershop')
))
...
-- ===========================
-- PART 8: TRIGGER FUNCTION FOR AUTO-CREATE BARBERSHOP_CUSTOMERS
-- ===========================

-- When user_profiles is created with role='customer', auto-create barbershop_customers
CREATE OR REPLACE FUNCTION auto_create_barbershop_customer()
RETURNS TRIGGER AS $$
BEGIN
  -- Only for customer role with phone number
...
-- Key changes:
-- 1. Removed foreign key constraint from user_profiles.customer_phone
-- 2. Added trigger to auto-create barbershop_customers record
-- 3. Fixed RLS policies for all 3 roles
-- 4. Added admin policies for management
-- =====================================================
