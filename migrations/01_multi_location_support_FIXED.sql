-- ============================================================================
-- BALIK.LAGI SYSTEM - MULTI-LOCATION SUPPORT MIGRATION (FIXED VERSION)
-- ============================================================================
-- 
...
-- 3.4: Add branch_id to barbershop_transactions table (if exists)
DO $$ 
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'public' AND table_name = 'barbershop_transactions'
    ) THEN
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns 
        ...
        END IF;
    ELSE
        RAISE NOTICE '⚠ barbershop_transactions table does not exist, skipping';
    END IF;
END $$;
...
-- 4.4: Update transactions with branch from capster (if table exists)
DO $$ 
DECLARE
    v_updated INTEGER;
...
BEGIN
    ...
    RAISE NOTICE '✓ RLS enabled, old policies dropped';
END $$;
