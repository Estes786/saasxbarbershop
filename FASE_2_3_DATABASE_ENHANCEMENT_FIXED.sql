-- =====================================================
-- FASE 2 & 3: DATABASE ENHANCEMENT (FIXED VERSION)
...
DO $$ 
BEGIN
  -- Add columns one by one to handle "already exists" errors gracefully
  ALTER TABLE bookings ADD COLUMN IF NOT EXISTS queue_number INTEGER;
  ALTER TABLE bookings ADD COLUMN IF NOT EXISTS estimated_start_time TIMESTAMPTZ;
  ALTER TABLE bookings ADD COLUMN IF NOT EXISTS estimated_duration_minutes INTEGER DEFAULT 30;
  ...
  ALTER TABLE bookings ADD COLUMN IF NOT EXISTS waiting_time_minutes INTEGER;
  ALTER TABLE bookings ADD COLUMN IF NOT EXISTS customer_notes TEXT;
  ALTER TABLE bookings ADD COLUMN IF NOT EXISTS capster_notes TEXT;
  
  -- Add booking_source with check constraint if not exists
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'bookings' AND column_name = 'booking_source'
  ) THEN
    ALTER TABLE bookings ADD COLUMN booking_source VARCHAR(50) DEFAULT 'online' CHECK (booking_source IN ('online', 'walk-in', 'phone'));
  END IF;
  
  ALTER TABLE bookings ADD COLUMN IF NOT EXISTS reminder_sent_at TIMESTAMPTZ;
  
  -- Add rating with check constraint if not exists
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'bookings' AND column_name = 'rating'
  ) THEN
    ALTER TABLE bookings ADD COLUMN rating INTEGER CHECK (rating BETWEEN 1 AND 5);
  END IF;
  
  ALTER TABLE bookings ADD COLUMN IF NOT EXISTS feedback TEXT;
END $$;

-- 2. Create indexes for booking queries
...
BEGIN
  ...
  v_confidence_score = LEAST(100, (v_total_visits * 10));
  ...
  -- Calculate predicted next visit date (add interval days to last visit date)
  IF v_avg_interval IS NOT NULL THEN
    v_result.predicted_next_visit_date = v_last_visit_date + (v_avg_interval * INTERVAL '1 day');
  ELSE
    v_result.predicted_next_visit_date = NULL;
  END IF;
...
-- 7. Populate historical visit data (only if transactions exist)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM barbershop_transactions LIMIT 1) THEN
...
-- 8. Generate initial predictions (only if visit history exists)
DO $$
DECLARE
  v_count INTEGER;
BEGIN
  IF EXISTS (SELECT 1 FROM customer_visit_history LIMIT 1) THEN
    v_count = update_all_customer_predictions();
    RAISE NOTICE 'Generated predictions for % customers', v_count;
  ELSE
    RAISE NOTICE 'No visit history data - skipping predictions';
  END IF;
END $$;
...
DROP POLICY IF EXISTS "customer_visit_history_select_own" ON customer_visit_history;
DROP POLICY IF EXISTS "customer_predictions_select_own" ON customer_predictions;
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '========================================';
  RAISE NOTICE 'FASE 2 & 3 DATABASE ENHANCEMENT COMPLETE!';
  RAISE NOTICE '========================================';
END $$;
